...
public class ProjectileEffect : AbilityEffect {
    ...
    public ProjectileEffect(string name, Actor actor, Vector3 targetPosition, ProjectileEffectData data)
    ...
    public override void action(AbilityCommand ability)
    {
        ...
        if(true)//angle != 0)
        {
            ...
            float length = Vector2.Distance(target2D, actor.get2DPos());
            Vector2 direction = (target2D - actor.get2DPos()).normalized;
            ...
            finalTarget = new Vector3(actor.get2DPos().x + newDirection.x, finalTarget.y, actor.get2DPos().y + newDirection.y);
        ...
        */
        ...
        if (data.weaponProjectile)
            projectileName = actor.getProjectileName();
        else
            projectileName = data.projectileName;

        Debug.Log("fire projectile " + data.name);
        GameMaster.getGameController().requestFireProjectile(actor.getID(), finalTarget, data.name, projectileName); //TODO: använd projectile name
    }
}
