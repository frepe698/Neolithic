...
public class ParticlePoolingManager
{
    ...
    private static volatile ParticlePoolingManager instance;

    //look up list of various object pools.
    private Dictionary<String, ParticlePool> particlePools;
    ...
    /// </summary>
    private ParticlePoolingManager()
    {
        //Ensure object pools exists.
        this.particlePools = new Dictionary<String, ParticlePool>();
    }
            ...
            if (instance == null)
            {
                ...
                lock (syncRoot)
                {
                    ...
                    if (instance == null)
                    {
                        ...
                        instance = new ParticlePoolingManager();
                    }
                }
            }
    ...
    /// <returns></returns>
    public bool CreatePool(GameObject particleObj, int targetPoolSize, bool shouldShrink)
    {
        //Check to see if the pool already exists.
        ParticleSystem particle = particleObj.GetComponent<ParticleSystem>();
        if (ParticlePoolingManager.Instance.particlePools.ContainsKey(particle.name))
        {
        ...
        else
        {
            ...
            ParticlePool nPool = new ParticlePool(particle, targetPoolSize, shouldShrink);
            //Add the pool to the dictionary of pools to manage
            //using the object name as the key and the pool as the value.
            ParticlePoolingManager.Instance.particlePools.Add(particle.name, nPool);
    ...
    public ParticleSystem GetObject(string name)
    {
        //Find the right pool and ask it for an object.
        return ParticlePoolingManager.Instance.particlePools[name].GetObject();
    }
}