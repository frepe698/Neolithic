...
public class ObjectPoolingManager
{
	...
	public bool CreatePool(GameObject objToPool, int targetPoolSize, bool shouldShrink)
	{
		...
		else
		{
			...
			ObjectPool nPool = new ObjectPool(objToPool, targetPoolSize, shouldShrink);
			//Add the pool to the dictionary of pools to manage
			//using the object name as the key and the pool as the value.
			ObjectPoolingManager.Instance.objectPools.Add(objToPool.name.ToLower(), nPool);
	...
	public GameObject GetObject(string objName)
	{
        ...
        if (!ObjectPoolingManager.Instance.objectPools.ContainsKey(objName.ToLower()))
        {
            Debug.LogWarning(objName + " is not in the pool");
            return null;
        }
		return ObjectPoolingManager.Instance.objectPools[objName.ToLower()].GetObject();
	}

	public void ReturnObject(string objName, GameObject nObj)
	{
		ObjectPoolingManager.Instance.objectPools[objName.ToLower()].ReturnObject(nObj);
	}
	

	public void ShrinkPool(string objName)
	{
//		ObjectPoolingManager.Instance.objectPools[objName.ToLower()].Shrink();
	}

	public void ShrinkPools()
	{
//		foreach(ObjectPool pool in ObjectPoolingManager.Instance.objectPools.Values)
//		{
//			pool.Shrink();
//		}
	}
